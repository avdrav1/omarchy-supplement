...
- Dotfiles and desktop/theme integration
  ...
  - `./install-shibumi.sh` – install the [Shibumi shell](https://github.com/HANCORE-linux/Shibumi-Shell) (the QS Rise V1/V2 layouts, controls, widgets and panels for the Omarchy shell), which replaced Lacuna on this fleet. Shibumi is not yet on the AUR, so this uses the project's documented **transitional source** install: `yay -S --needed` the runtime deps (`python jq curl networkmanager power-profiles-daemon upower xdg-utils libnotify wl-clipboard` plus fonts `ttf-material-symbols-variable ttf-jetbrains-mono-nerd-basic noto-fonts-cjk adwaita-fonts`), clone/`git pull` the repo into `~/.local/share/shibumi-shell` (override with `SHIBUMI_SRC`/`SHIBUMI_REPO`), then run `scripts/shibumi-suite install --yes` to stage the ~24 user plugins into `~/.config/omarchy` (user-scoped, no sudo). Also points Omarchy's default terminal at `ghostty` (override `SHIBUMI_TERMINAL`; rewrites `~/.config/xdg-terminals.list`), carried over from the old Lacuna setup. Re-runnable: it pulls the checkout and re-stages, which restores plugins to canonical each time. Ends with `omarchy-restart-shell` when a Wayland session is up (skipped in a TTY/first-boot context). **When shibumi-shell lands on the AUR**, collapse the source dance to `omarchy pkg aur add shibumi-shell && shibumi-shell install --yes` and mirror the old lacuna installer's `pacman -Qi` + CLI shape.
