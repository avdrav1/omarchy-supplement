...
- Dotfiles and desktop/theme integration
  ...
  - `./install-theme.sh` – apply the Dos-Moos Omarchy theme, restart snappy-switcher so it picks up its themed config, then install the Quickshell Rise bar from `https://github.com/HANCORE-linux/quickshell-dots` via the official installer (`V1 --claude-backend --autostart`). **No quickshell package work**: upstream only requires `qs` on `PATH`, so Rise runs against whatever quickshell is installed — including the `quickshell-git` that Omarchy 4's own shell depends on. The script installs upstream's prerequisites itself (`git jq curl pacman-contrib bluez-utils` + the JetBrainsMono Nerd / Material Symbols fonts) so the one privileged step is predictable, instead of upstream hitting a sudo prompt mid-run inside `set -e`. `--autostart` installs the post-boot hook and, on Omarchy 4 (quattro), hides the stock omarchy bar — a state Rise tracks as its own and returns on `--no-autostart`. This *used* to force-remove quickshell and build a pinned `mainstream-quickshell-git` from `quickshell/PKGBUILD`, which conflicted with `omarchy-bar` and made the bar Omarchy-3-only; that path was removed after verifying Rise on Omarchy 4 (2026-08-05), leaving `quickshell/PKGBUILD` and `quickshell/quickshell-check.hook` unused.
